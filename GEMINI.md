...
- **Tech Stack:**
  ...
  - **Backend:** Express, PostgreSQL (pg-pool), Drizzle ORM, AI SDK (Vercel AI), Stripe.
  - **Database:** PostgreSQL with migrations managed by Drizzle (`server/drizzle`).
- **Key Features:**
...
3. Start the required databases (PostgreSQL & Redis):
   ```bash
   docker-compose up -d
   ```
...
- **Database Migrations:** SQL files generated by Drizzle Kit in `server/drizzle` are automatically applied on server startup. Generate new ones using `npx drizzle-kit generate`.
