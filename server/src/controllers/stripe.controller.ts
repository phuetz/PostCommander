...
export const createCheckout = catchAsync(async (
  req: Request,
  res: Response,
): Promise<void> => {
  ...
  const cancelUrl =
    process.env.STRIPE_CANCEL_URL || 'http://localhost:5173/pricing';
...
export const createPortal = catchAsync(async (
  req: Request,
  res: Response,
): Promise<void> => {
  const requestUser = requireRequestUser(req);
  ...
  const db = getDrizzle();
  const [user] = await db.select().from(usersTable).where(eq(usersTable.id, requestUser.id)).limit(1);
  ...
  const returnUrl =
    process.env.STRIPE_SUCCESS_URL || 'http://localhost:5173/app/billing';
...
export const getSubscriptionStatus = catchAsync(async (
  req: Request,
  res: Response,
): Promise<void> => {
  const requestUser = requireRequestUser(req);
  ...
  if (user.postsResetDate && new Date(user.postsResetDate) <= new Date()) {
    ...
    await db.update(usersTable)
      .set({ postsUsedThisMonth: 0, postsResetDate: nextReset, updatedAt: new Date().toISOString() })
...
export const cancelUserSubscription = catchAsync(async (
  req: Request,
  res: Response,
): Promise<void> => {
  const requestUser = requireRequestUser(req);
...
export const resumeUserSubscription = catchAsync(async (
  req: Request,
  res: Response,
): Promise<void> => {
  const requestUser = requireRequestUser(req);
  ...
  const response: ApiResponse<{ message: string }> = {
    success: true,
    data: { message: 'Subscription cancellation has been reversed. Your subscription will continue.' },
  };
...
export const stripeWebhookHandler = catchAsync(async (
  req: Request,
  res: Response,
): Promise<void> => {
...
export const getInvoices = catchAsync(async (
  req: Request,
  res: Response,
): Promise<void> => {
  const requestUser = requireRequestUser(req);
...
export const getPlansHandler = catchAsync(async (
  _req: Request,
  res: Response,
): Promise<void> => {
